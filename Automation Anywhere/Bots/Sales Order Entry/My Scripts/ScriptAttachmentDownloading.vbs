...
For i = 1 To MailItems.Count  
	If MailItems.Item(i).Unread = true then
		...
		If InStr(vSubject,"TRIGGER") <> 0 AND InStr(vSubject,"PO") <> 0 AND InStr(vSubject,"ENTRY") <> 0 AND InStr(vSubject,"BOT") <> 0 AND InStr(vSubject,"TEST") <> 0then
			...
			'==Loop through all Valid Email Addresses Begins=
		...
		End if
		'End of Subject includes If Statement
	End if
